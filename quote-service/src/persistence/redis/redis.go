...
type RedisCli interface {
	...
	SMembers(key string, scanSlice interface{}) error
...
func (r *Client) SMembers(key string, scanSlice interface{}) error {
	res := r.cli.SMembers(key)
	if res.Err() == redis.Nil {
		// it means that key does not exist in redis
		return nil
	}

	return res.ScanSlice(scanSlice)
}
